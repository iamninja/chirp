...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship based on user_id and friend_id works" do
  	...
  	assert users(:ro).pending_friends.include?(users(:ro1))
  end

  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:ro), friend: users(:ro2)
    end
  ...
  context "#send_request_email" do
    setup do
      @user_friendship = UserFriendship.create user: users(:ro), friend: users(:ro2)
    end
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:ro), users(:ro1)
      @friendship1 = users(:ro).user_friendships.where(friend_id: users(:ro1).id).first
      @friendship2 = users(:ro1).user_friendships.where(friend_id: users(:ro).id).first
    end
  ...
  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:ro), users(:ro2)
    end
  ...
  context ".request" do
    should "create two user friendships" do
      assert_difference "UserFriendship.count", 2 do
        UserFriendship.request(users(:ro), users(:ro2))
      end
    end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:ro), users(:ro2)
      assert !uf.save
    end
  end
end
